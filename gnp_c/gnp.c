...
/* use mp and mpi to construct the model */
void fitModel() {
  int i, j;
  char buf[100];
  FILE *f;
...
void read_basic_data() {
  ...
  dt2p = (float **)malloc(sizeof(void *)*nt);
  for (i=0; i<nt; i++) {
    dt2p[i] = (float *)malloc(sizeof(float)*np);
  }
  tip = (unsigned long *)malloc(sizeof(unsigned long)*nt);
